...
--     ∫ (x : ℝ × ℝ) in Set.Ioo 0 1 ×ˢ Set.Ioo 0 1, eval x.1 (shiftedLegendre n) *
--     eval x.2 (shiftedLegendre n) * -(x.1 * x.2).log / (1 - x.1 * x.2) =
--     ∫ (x : ℝ × ℝ) in Set.Ioo 0 1 ×ˢ Set.Ioo 0 1, eval x.1 (shiftedLegendre n) *
--     eval x.2 (shiftedLegendre n) * ∫ (z : ℝ) in (0)..1, 1 / (1 - (1 - x.1 * x.2) * z) := by
...
-- lemma zeta_3_eq_form : ∫ (x : ℝ × ℝ × ℝ) in Set.Ioo 0 1 ×ˢ Set.Ioo 0 1 ×ˢ Set.Ioo 0 1,
--     1 / ((1 - (1 - x.2.2) * x.1) * (1 - x.2.1 * x.2.2)) = 2 * ∑' n : ℕ , 1 / ((n : ℝ) + 1) ^ 3 := by
--   sorry
--   calc
...
--     obtain h := zeta_3
--     simp only [J, pow_zero, mul_one] at h
--     rw [← h]
--     symm
--     rw [← MeasureTheory.integral_indicator (by measurability), MeasureTheory.Measure.volume_eq_prod,
--       MeasureTheory.integral_prod]
--     · rw [intervalIntegral.integral_of_le (by norm_num),
--         MeasureTheory.integral_Ioc_eq_integral_Ioo, ← MeasureTheory.integral_indicator (by measurability)]
--       congr
--       ext x

--       sorry
--     · apply MeasureTheory.IntegrableOn.integrable_indicator _ (by measurability)
--       have := integrableOn_J_rr 0
