...
noncomputable abbrev I (r s : ℕ) : ENNReal :=
  ∫⁻ (x : ℝ) in Set.Ioo 0 1,
    ∫⁻ (y : ℝ) in Set.Ioo 0 1, ENNReal.ofReal (x ^ r * y ^ s / (1 - x * y))

noncomputable abbrev J (r s : ℕ) : ENNReal :=
  ∫⁻ (x : ℝ) in Set.Ioo 0 1,
    ∫⁻ (y : ℝ) in Set.Ioo 0 1,
      ENNReal.ofReal (-x ^ r * y ^ s * (x * y).log / (1 - x * y))
...
-- lemma zeta3_integrable :
--   MeasureTheory.IntegrableOn (fun x ↦ Real.log (x.1 * x.2) / (1 - x.1 * x.2)) (Set.Ioo 0 1 ×ˢ Set.Ioo 0 1)
...
lemma zeta3_aux :
    ...
    ∫⁻ (x : ℝ × ℝ) in (Set.Ioo 0 1 ×ˢ Set.Ioo 0 1),
      ENNReal.ofReal (-(x.1 * x.2).log / (1 - x.1 * x.2)) := by
  sorry
--   delta J
...
--     intro s hs
--     -- simp only [one_div, Filter.mem_atTop_sets, ge_iff_le, Set.mem_preimage] at hs ⊢
--     rw [nhds_def] at hs
--     refine Filter.iInf_sets_induct (hs := hs) ?_ ?_
--     · simp only [one_div, Set.preimage_univ, Filter.mem_atTop_sets, ge_iff_le, Set.mem_univ,
--       implies_true, exists_const]
--     · intro i s1 s2 hs1 hs2
--       simp only [Set.mem_setOf_eq, one_div, Filter.mem_atTop_sets, ge_iff_le,
--         Set.mem_preimage] at hs1 hs2
--       set p := _
--       change s1 ∈ ⨅ (_ : p), _ at hs1
--       by_cases hp : p
--       · rw [iInf_pos (hp := hp)] at hs1
--         simp only [Filter.mem_principal] at hs1
--         obtain ⟨a, ha⟩ := hs2
--         simp only [one_div, Set.preimage_inter, Filter.mem_atTop_sets, ge_iff_le, Set.mem_inter_iff,
--           Set.mem_preimage]
--         obtain ⟨hp1, hp2⟩ := hp
--         by_cases hx : (x1, x2) ∈ Set.Ioo 0 1 ×ˢ Set.Ioo 0 1
--         · sorry
--         · sorry
--       · rw [iInf_neg (hp := hp)] at hs1
--         simp only [Filter.mem_top] at hs1
--         simp only [one_div, hs1, Set.univ_inter, Filter.mem_atTop_sets, ge_iff_le, Set.mem_preimage,
--           hs2]    -- rw [Filter.mem_iInf] at hs
--     -- obtain ⟨I, hI, ⟨V, hV, rfl⟩⟩ := hs
--     -- -- simp only [Set.mem_setOf_eq, Subtype.forall] at hV
--     -- apply hI.induction_on
--     -- rintro b -
--     -- rintro _ ⟨i, rfl⟩
--     -- simp only
--     -- specialize hV i.1 i.2
--     -- set p := _
--     -- change V i ∈ ⨅ (_ : p), _ at hV
--     -- by_cases hp : p
--     -- · rw [iInf_pos (hp := hp)] at hV
--     --   simp only [Filter.mem_principal] at hV
--     --   apply hV
--     --   have hp1 := hp.1
--     --   by_cases hx : (x1, x2) ∈ (Set.Ioo (b + 2 : ℝ)⁻¹ (1 - (b + 2 : ℝ)⁻¹) ×ˢ Set.Ioo (b + 2 : ℝ)⁻¹ (1 - (b + 2 : ℝ)⁻¹))
--     --   · rw [Set.indicator_apply, if_pos hx]
--     --     rwa [Set.indicator_apply, if_pos] at hp1

--     --     sorry
--     --   · rw [Set.indicator_apply, if_neg hx]
--     --     rw [Set.indicator_apply] at hp1

--     --     sorry
--     -- · rw [iInf_neg (hp := hp)] at hV
--     --   simp only [Filter.mem_top] at hV
--     --   rw [hV]
--     --   trivial
...
theorem zeta_3 : J 0 0 = 2 * ∑' n : ℕ , 1 / ((n : ENNReal) + 1) ^ 3 := by
  rw [zeta3_aux]
